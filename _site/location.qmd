...
The chart below displays the geographical targeting of political advertisers on Meta platforms.
...
htmltools::browsable(
  ...
  columns = list(
    amount_spent = colDef(
      name = "💰 Spending",
      format = colFormat(prefix = "kr", separators = TRUE, digits = 0),
...
For example, if one ad targets two ZIP codes with $100 and another targets just one of those ZIP codes with kr100, we can’t separate the overlap. In this case, we record kr200 spent on the shared ZIP code and kr100 on the other.
...
For example, if one ad targets two ZIP codes with kr100 and another targets just one of those ZIP codes with kr100, we can’t separate the overlap. In this case, we record kr200 spent on the shared ZIP code and kr100 on the other.
